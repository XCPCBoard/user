...
//@param post 评论结构体指针
func CreatePostService(post *entity.Post) *errors.MyError {
	res := dao.DBClient.Create(post)
	if res.Error != nil {
		logger.Logger.Error("create post error", res.Error, 0, fmt.Sprintf("%+v", post))
...
//@param post 评论参数
func UpdatePostService(post *entity.Post) *errors.MyError {
	////检查主键是否为0,gorm特性是id为
	//if post.Id == 0 {
	//	err := errors.New("post's id should not be 0")
	//	logger.Logger.Error("post's id should not be 0", err,0, fmt.Sprintf("post: %+v", post))
	//	return err
	//}
...
func SelectPostService(id string, post *map[string]interface{}) *errors.MyError {
